...
public interface ItemSearchApi {
   /**
	 * 
	 * @api {post} http://47.93.48.111:6181/api/item/itemSearch   （前台）首页搜索框查询
	 * @apiName itemSearch
	 * @apiGroup itemSearch
	 * @apiVersion 0.1.0
	 * @apiDescription 首页搜索框,输入关键词进行查询
	 * 
	 * @apiParam {Integer} rule   1.时间降序2.销量降序3.价格降序4.价格升序(非必须)
	 * @apiParam {String} keyWord  搜索关键词,推荐使用牙,牙具,牙膏,牙套,小牙套(必须)
	 * @apiParam {String} currentPage  当前第几页,由于数据少,默认从第一页开始(必须,默认为1)
	 * @apiParam {String} numberPerPage  每页显示的记录的条数 (非必须)
	 * 
