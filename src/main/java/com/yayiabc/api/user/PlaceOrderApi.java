...
public interface PlaceOrderApi {
	/**
	 * @api {post} http://47.93.48.111:8080/api/po/generaOrder  提交订单
	 * @apiName generaOrder
	 * @apiGroup po
	 * @apiVersion 0.1.0
	 * @apiDescription 提交订单
	 ...
	 * 
	 * @apiParam {Integer} qbnum    钱币数  （非必须）
	 * @apiParam {String} token     （必须） 
	 * 
	 * @apiSuccessExample {json} Success-Response:
	 *  HTTP/1.1 200 OK
	 ...
	 * 
	 * @apiParam {Integer} receiverId    (必须)
	 * @apiParam {Integer} sumPrice (必须)商品总价
	 * @apiParam {Integer} itemSum (必须)商品总数目
	 * 
	 * @apiSuccessExample {json} Success-Response:
	 *  HTTP/1.1 200 OK
	 * {
	 ...
	 * @apiDescription 提交订单！！！
	 * 
	 * @apiParam {String} Ok  (这里的参数格式已经和带费力沟通好了 并测试过 这里就不一一写了) 
	 *
	 * @apiSuccessExample {json} Success-Response:
	 *  HTTP/1.1 200 OK
	 * {
