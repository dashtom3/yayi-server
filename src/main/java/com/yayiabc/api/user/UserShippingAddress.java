...
public class UserShippingAddress {
     ...
     * 
     * @apiParam {String} token （必须 用来查询当前用户的userId 以便添加收货地址）
     ...
     * 
	 * @apiParam {String} token （必须 用来查询当前用户的userId 以便修改收货地址）
     * @apiParam {String} receiverId  收货地址表Id (必须)
	 * @apiParam {String} province （省份） 
     * @apiParam {String} city     （城市）
     * @apiParam {String} county   （县城） 
     * @apiParam {String} receiverName   (收货人姓名) 
     * @apiParam {String} landlineNumber    座机号(非必须)
	 * @apiParam {String} receiverDetail   （详细地址） (非必须)
     ...
     * 
	 * @apiParam {String} token （必须 用来查询当前用户的userId 以便显示收货地址）
	 * 
     * @apiSuccessExample {json} Success-Response:
     *  HTTP/1.1 200 OK
     * {
