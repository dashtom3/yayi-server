...
public class TestWeiXinPay {
	public static void main(String[] args) throws Exception{
		...
		reqData.put("out_trade_no", "1565749852852");//out_trade_no不要传人你自己订单系统的订单号，而是重新生成一个新的唯一流水，这样每次请求提交支付，可以保证out_trade_no不重复。
		reqData.put("total_fee", "1");//必传,总金额,接口中单位为分,对账单中的单位为元,必须为整数,可以通过参数传进来
		reqData.put("spbill_create_ip","127.0.0.1");//终端ip,必传,APP和网页支付提交用户端ip，Native支付填调用微信支付API的机器IP。
